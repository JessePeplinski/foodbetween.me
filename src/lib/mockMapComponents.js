...
export class MockGoogleMapsAPI {
  ...
  createMockMapsAPI() {
    return {
      Map: MockMap,
      Marker: MockMarker,
      Circle: MockCircle, // Add the Circle class
      places: {
...
class MockCircle {
  constructor(options) {
    console.log('[MOCK] Creating circle with options:', options);
    this.options = options;
    this.map = options.map;
    this.center = options.center;
    this.radius = options.radius;
    this.fillColor = options.fillColor || 'rgba(66, 133, 244, 0.1)';
    this.fillOpacity = options.fillOpacity || 0.3;
    this.strokeColor = options.strokeColor || '#4285F4';
    ...
    if (this.map) {
      if (!this.map.circles) this.map.circles = [];
      this.map.circles.push(this);
      this.renderMockCircle();
    }
  }
  ...
  renderMockCircle() {
    ...
    circleElement.style.border = `${this.strokeWeight}px solid ${this.strokeColor}`;
    circleElement.style.opacity = `${this.strokeOpacity}`;
    circleElement.style.backgroundColor = this.fillColor;
    circleElement.style.transform = 'translate(-50%, -50%)';
    circleElement.style.pointerEvents = 'none';
    circleElement.style.zIndex = '5';
    
    // Position the circle in the center of the map
    circleElement.style.top = '50%';
    circleElement.style.left = '50%';
    
    // Size the circle based on radius - in our mock implementation, 
    // we'll make the radius take up to 80% of the map width for visualization
    const mapWidth = this.map.element.offsetWidth;
    const scaleFactor = 0.8; // 80% of the map size
    const size = mapWidth * scaleFactor;
    ...
    radiusLabel.style.position = 'absolute';
    radiusLabel.style.bottom = '5px';
    radiusLabel.style.right = '5px';
    radiusLabel.style.backgroundColor = 'rgba(255, 255, 255, 0.8)';
    radiusLabel.style.padding = '2px 6px';
    radiusLabel.style.borderRadius = '4px';
    radiusLabel.style.fontSize = '12px';
    radiusLabel.style.fontFamily = 'sans-serif';
    radiusLabel.style.color = '#333';
    
    // Format radius for display (convert to km/miles)
    let radiusText = '';
    if (this.radius >= 1000) {
      radiusText = `${(this.radius / 1000).toFixed(1)} km`;
    } else if (this.radius >= 1609) {
      radiusText = `${(this.radius / 1609).toFixed(1)} mi`;
    } else {
      radiusText = `${this.radius} m`;
    }
    
    radiusLabel.textContent = `Radius: ${radiusText}`;
    circleElement.appendChild(radiusLabel);
    
    this.map.element.appendChild(circleElement);
    this._element = circleElement;
  }
  
  setMap(map) {
    console.log('[MOCK] Setting circle map:', map ? 'map instance' : 'null');
    this.map = map;
    
    // Remove the circle element if it exists
    if (this._element && this._element.parentNode) {
      this._element.parentNode.removeChild(this._element);
    }
